 ...
 * 限制属性名长度，防止ReDoS
 */
export const sanitizeProperty = (prop: string): string => {
 ...
 * 检查属性是否匹配给定的模式列表
 */
export const isPropMatch = (prop: string, properties: string[]): boolean => {
 ...
 * 检查是否包含 px-ignore 注释
 */
export const isPxIgnore = (decl: Declaration): boolean => {
 ...
 * 检查是否是负值 px
 */
export const isMinusPx = (decl: Declaration): boolean => {
 ...
 * 检查选择器是否在黑名单中
 */
export const checkSelectorBlackList = (selector: string, blacklist: (string | RegExp)[]): boolean => {
  if (!selector || !blacklist || blacklist.length === 0) return false;
